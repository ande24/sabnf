...
export const BetDraw = ({ onClose, onPrint }: BetDrawProps) => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      await setDoc(doc(collection(db, 'transactions'), `${docId}_bet`), {
        ...
        fight_number: adminConfig.current_fight,
        side: 'DRAW',
        reason: `Bet placed on DRAW - Fight #${adminConfig.current_fight}`,
      });
      ...
      if (fightDocSnapshot.exists()) {
        ...
        const updatedDrawTotal = (fightDocData.draw_total || 0) + Number(betAmount);
        await updateDoc(fightDocRef, {
          draw_total: updatedDrawTotal,
        });
        console.log('Fight document updated with new draw_total:', updatedDrawTotal);
      } else {
      ...
      <View className="w-11/12 rounded-2xl bg-zinc-800">
        ...
        <View className="w-full rounded-t-2xl">
          <StyledGradient
            ...
            <Text style={{ fontFamily: "Orbitron-bold"}} className="text-4xl tracking-widest text-white">DRAW</Text>
            <TouchableOpacity
        ...
        <StyledGradient
          ...
          <View className="w-full flex-row justify-between items-center py-3">
            ...
            <View className="flex-row items-center gap-x-2 rounded-md border border-zinc-600 px-3 py-1">
              ...
              <Text className="text-sm text-green-400">
                {formatNumber(fightData.draw_total || 0)}
              </Text>
            </View>
          </View>
          ...
          <View className="mb-4 h-14 flex-row items-center justify-center">
            <TouchableOpacity
              className="flex h-full flex-[2] items-center justify-center rounded-l-md bg-green-600"
            ...
            <TouchableOpacity
              className="flex h-full flex-[2] items-center justify-center rounded-r-md bg-green-600"
          ...
          <TouchableOpacity
            ...
            className="mb-4 w-full rounded-md bg-green-500 p-[2px]">
            <StyledGradient
              colors={['#047857', '#059669']}
              start={{ x: 0, y: 0 }}
              end={{ x: 0, y: 1 }}
