...
export const BetMeron = ({ onClose, onPrint }: BetMeronProps) => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      await setDoc(doc(collection(db, 'transactions'), `${docId}_bet`), {
        ...
        fight_number: adminConfig.current_fight,
        side: 'MERON',
        reason: `Bet placed on MERON - Fight #${adminConfig.current_fight}`,
      });
      ...
      const formattedDate = nowDate.toLocaleDateString('en-PH', { year: 'numeric', month: 'long', day: 'numeric', timeZone: 'Asia/Manila' });
      ...
      <View className="w-11/12 rounded-2xl bg-zinc-800">
        ...
        <StyledGradient
          ...
          <View className="w-full flex-row justify-between items-center py-3">
            ...
            <View className="flex-row items-center gap-x-2 rounded-md border border-zinc-600 px-3 py-1">
              ...
              <Text className="text-sm text-red-500">
                {formatNumber(fightData.meron_total)}
              </Text>
            </View>
          </View>
          ...
          <TouchableOpacity
            ...
            className="mb-4 w-full rounded-md bg-red-700 p-[2px]">
            <StyledGradient
              colors={['#7f1d1d', '#d81d1d']}
              start={{ x: 0, y: 0 }}
              end={{ x: 0, y: 1 }}
              ...
              <Text
                style={{ fontFamily: 'Orbitron-bold' }}
                className="text-center text-3xl tracking-widest text-white">
                SUBMIT
              </Text>
            </StyledGradient>
          </TouchableOpacity>
